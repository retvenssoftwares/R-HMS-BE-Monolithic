...
const editDiscountPlan = async (req, res) => {
    try {
        const { userId, discountName, shortCode, deviceType, discountType, discountPercent, newRatePlanName, blackOutDates, discountPrice, validityPeriodFrom, validityPeriodTo, discountTotal, extraAdultRate, extraChildRate, ratePlanInclusion, displayStatus, ipAddress } = req.body
        ...
        if (result.success) {
            ...
            if (newRatePlanName) {
                const newRatePlanNameObject = {
                    newRatePlanName: newRatePlanName,
                    logId: Randomstring.generate(10),
                };
                findDiscountPlan.newRatePlanName.unshift(newRatePlanNameObject);
            }
            if (discountType) {
