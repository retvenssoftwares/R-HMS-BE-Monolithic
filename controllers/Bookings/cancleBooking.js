...
export const cancelBooking = async (req, res) => {
  const { reservationIds , propertyId } = req.body;

  try {
    const detailsPromises = reservationIds.map(async (item) => {
      console.log(item)
      // Find the document and include the version (__v) in the query
      const existingDocument = await confirmBooking.findOneAndUpdate(
        { reservationId: item  , propertyId},
    ...
    if(data){
      return res.status(200).json({message : "booking cancle successfully" , statusCode : 200})
    }
